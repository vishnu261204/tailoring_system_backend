...
class UpdateOrderRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $orderDate = $order instanceof Order ? $order->order_date->format('Y-m-d') : null;

        return [
            'trial_date' => ['nullable', 'date', 'after_or_equal:'.$orderDate],
            'delivery_date' => ['nullable', 'date', 'after_or_equal:trial_date'],
            'advance_amount' => ['nullable', 'numeric', 'min:0'],
            'items' => ['nullable', 'array', 'min:1'],
            'items.*.item_type' => ['required_with:items', 'string', 'in:'.self::ITEM_TYPES],
